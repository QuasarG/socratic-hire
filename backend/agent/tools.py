...
from backend.agent.prompts import STATE_SNAPSHOT_HEADER
from backend.core.llm_client import call_llm_json
...
def resolve_blueprint(sess: dict) -> dict | None:
    ...
    from backend.rag.jobs_store import find_by_title, full_job
...
def _ask_question(ctx: ToolContext, args: dict) -> dict:
    ...
    sess = state.get_session(ctx.session_id)
...
def _search_jobs(ctx: ToolContext, args: dict) -> dict:
    ...
    except Exception as exc:  # 检索坏了不伤主链路
        return {"ok": False, "jobs": [], "note": f"检索不可用：{exc}，请直接裸问"}
    # 摘要截断：前端岗位卡片直接展示 excerpt，控制 detail 体积
    for j in jobs:
...
__all__ = ["TOOLS", "TOOLS_BY_NAME", "TOOL_RESULT_MAX_CHARS", "ToolContext", "tools_schema", "state_snapshot"]
